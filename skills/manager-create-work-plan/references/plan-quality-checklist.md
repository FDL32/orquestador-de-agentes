...
- [ ] La seccion `Decision Arquitectonica` existe y no esta vacia. `validate_ticket_prose.py:404` (TP-PROSE-10) la exige igual que exige `Non-goals` en `:221` (TP-PROSE-06, con umbral de 30 caracteres de contenido). "Ninguna, y por que" es una respuesta VALIDA para un cambio local.
- [ ] **Ninguna de esas dos secciones conserva el placeholder de la plantilla.** Este check lo hace el REVISOR HUMANO, y no hay barrera detras: MEDIDO 2026-09-08, el placeholder sin editar supera los 30 caracteres y `validate_ticket_prose` lo da por VERDE sin encender ningun aviso, TP-PROSE-02 incluido. El relleno ritual es invisible para la maquina -- si tu solo compruebas que la seccion "existe y no esta vacia", este par de secciones simula gobierno en vez de ejercerlo. Senal barata: si el texto sigue entre corchetes `[...]`, no se ha escrito.
