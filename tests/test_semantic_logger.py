...
from hooks.native_post_tool_hook import main as native_hook_main  # noqa: E402
from hooks.post_tool_hook import (  # noqa: E402
    log_observation,
    post_tool_hook,
    reset_counter,
)
...
class TestSemanticLogger:
    ...
    def teardown_method(self):
        ...
        try:
            if (
                hasattr(self, "test_observations_file")
                and self.test_observations_file.exists()
            ):
    ...
    def test_hook_translation(self):
        ...
        with (
            ...
            patch("sys.stdout"),
            patch("hooks.post_tool_hook.post_tool_hook") as mock_post_hook,
        ):
            native_hook_main()

            # Verificar que se llamÃ³ con contexto completo
            mock_post_hook.assert_called_once()
            args, _kwargs = mock_post_hook.call_args
            context = args[0]
    ...
    def test_append_only(self):
        ...
        with (
            ...
            with open(self.test_observations_file, encoding="utf-8") as f:
                lines = f.readlines()
                assert len(lines) == 1
