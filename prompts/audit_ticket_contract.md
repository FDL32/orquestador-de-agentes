...
>
> Audita el contrato operativo del ticket (`work_plan.md` + estrategia/audit)
> ANTES de pasar a Builder. No confundir con review de implementación, bus,
...
- La evidencia de commit debe referenciar exactamente el ticket activo
  (`WOT-...a` incluido si aplica). Un commit con ID truncado o ticket distinto
...
|---|---|
| `work_plan.md` ↔ `STRATEGY_WOT-*` | Files Likely Touched, TPs, fases y scope (legacy-compat: `PLAN_WT-*`, `PLAN_WP-*`) |
| `STRATEGY_WOT-*` ↔ `AUDIT_WOT-*` | Criterios de aceptación expresados de forma verificable (legacy-compat: `PLAN_WT-*`/`AUDIT_WT-*`) |
