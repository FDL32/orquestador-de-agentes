...
8. Con `--session-close` en verde y el arbol limpio, materializa los follow-ups PREPARADOS en 1.3.7: **aplica primero el DEDUPE de tres superficies (definido en 8.bis, ver abajo)**, escribe fila Vista rapida + ficha en `backlog.md`, corre `check_backlog_contract.py`, reporta `[REGISTRADO en <ruta>:<ticket>]`. NO commitees. Registrar follow-ups nunca debe poder bloquear el cierre que los origino.
   - **DEDUPE OBLIGATORIO, identico al de 8.bis (WOT-2026-047w).** Antes de escribir CADA follow-up propio, aplica **la misma clausula DEDUPE que define 8.bis** sobre las MISMAS tres superficies (`backlog.md` vivo, `_archive/backlog_done.md` y el archive de memoria `observations.YYYY-MM.jsonl`), con la MISMA regla de lectura completa de cada hit. **El criterio vive UNA sola vez, en la clausula DEDUPE de 8.bis; esta linea REMITE a ella, no la redefine** (si divergieran, prevalece 8.bis).
   - **Por que estaba aqui el agujero, y no en 8.bis:** el paso 8 es la via por la que una sesion ficha SUS PROPIOS hallazgos -- la mas frecuente con diferencia -- y era exactamente la que no comprobaba si el hallazgo ya estaba fichado. La leccion del incidente fundacional (2026-07-22, 4 duplicados en un dia, **tres de ellos declarando haber buscado duplicados**) se escribio en `memory_upload.md:18-52` y en 8.bis, pero NO aqui.
   - **Salida EXACTA y obligatoria por cada follow-up**, del vocabulario cerrado: `[DEDUPE: YA CUBIERTO por <WOT-id> -> no se da de alta]`, `[DEDUPE: AMPLIAR <WOT-id> -> se amplia la ficha existente, no se crea fila]` o `[DEDUPE: SIN VECINOS -> alta nueva]`. Un follow-up registrado sin una de estas tres marcas es un alta sin dedupe.
   - **Ningun guard del arbol caza esto:** `check_backlog_contract.py` detecta duplicados solo por id EXACTO, asi que dos ids distintos con el mismo contenido pasan. El dedupe es NORMA asistida por senal, no barrera; por eso la marca de salida es obligatoria.

8.bis FUSIoN DEL `backlog_inbox/` (tickets preparados por SESIONES DE DISEnO paralelas). Ademas de tus propios follow-ups (paso 8), fusiona las fichas que una sesion de diseno read-only dejo en `<destination_root>/.agent/collaboration/backlog_inbox/*.tickets.md` (buzon de un-escritor: solo diseno escribe ahi, solo tu -desarrollo- escribes `backlog.md`; asi no hay carrera). Por cada ficha, con el arbol ya limpio y post `--session-close`:
   - Aplica el MISMO gate de evidencia que el paso 8 (una ficha sin evidencia verificable no se fusiona: se deja o se degrada).
   - DEDUPE (AMBAS superficies, lectura completa): comprueba que no exista ya una fila equivalente en `backlog.md`; si existe, vincula/absorbe, no dupliques (la ficha declara el ticket vivo relacionado cuando lo hay). El barrido cubre `backlog.md` vivo, `_archive/backlog_done.md` Y el archive de memoria (`.agent/runtime/memory/archive/observations.YYYY-MM.jsonl`): medido 2026-07-22, un "hallazgo" que ya era una ficha del cierre anterior paso porque el dedupe se hizo SOLO contra el archive de memoria y nunca contra el backlog. LEE ENTERO cada hit: no descartes por el TITULO ni por el id -- las lecciones se redactan como reglas ABSTRACTAS y los candidatos como casos CONCRETOS, asi que los titulos no se parecen aunque el contenido sea el mismo (ese fue el 3er duplicado del mismo dia: la entrada aparecio en los hits y se descarto por su titulo). Ayuda mecanica opcional, generador de SENAL y nunca veredicto: `python scripts/find_similar_signals.py --text-file <candidato> --archive <motor>/.agent/runtime/memory/archive/observations.2026-07.jsonl --backlog <destination_root>/.agent/collaboration/backlog.md --backlog <destination_root>/.agent/collaboration/_archive/backlog_done.md` (no decide, no bloquea, siempre exit 0 con o sin vecinos).
   - Asigna el SIGUIENTE WOT-id libre contra `backlog.md` RE-LEiDO + el set `reserved_ids` de esta misma ejecucion (los ids que el paso 8 ya asigno): asi el paso 8 y este 8.bis no calculan "siguiente libre" contra el mismo snapshot y no colisionan. Escribe la fila `Vista rapida` con las OCHO celdas obligatorias en orden (`Prioridad | Ticket | Titulo | Scope | Estado | Depende de | Origen | Reactivation`; Estado del vocabulario cerrado -- `pending` para un ticket nuevo; Reactivation `-` salvo estados que exijan trigger) + la ficha detallada; corre `check_backlog_contract.py` (si falla, la fila esta mal formada: corrigela, no fuerces el cierre) y BORRA la ficha fusionada del inbox. Reporta `[FUSIONADO desde backlog_inbox: <FP-...> -> <WOT-id>]`.
...
8.ter CIERRE DEL ESTADO DE LOS TICKETS TRABAJADOS (WOT-2026-026t). Los pasos 8 y 8.bis solo AÑADEN filas (follow-ups nuevos, fichas del inbox); ninguno cierra el estado de lo que ESTA sesion entrego. Ese hueco es el que produjo la fuga medida, asi que el cierre de estado es un paso PROPIO, no un efecto colateral de los anteriores.
   ...
   - **Sin evidencia de aterrizaje no se archiva.** Mismo umbral que el paso 8: el `commit:<sha>` debe ser ANCESTRO de HEAD, no un sha suelto. Comando exacto, con el repo explicito (un sha del motor NO se comprueba contra el destino): `git -C <repo_donde_vive_el_commit> merge-base --is-ancestor <sha> HEAD` -> `rc=0` significa que aterrizo. Para verificar la COLUMNA entera de una tacada existe `scripts/check_backlog_commits_landed.py`, que ademas conoce su propio denominador. Si el trabajo se hizo sin citar el id, dilo en `Reactivation` con un trigger estructurado (`condition:<slug>`) y su verificacion, en vez de inventar un commit.
   - **Verifica por comando, nunca de memoria:** `python <MOTOR_ROOT>/scripts/check_backlog_contract.py --project-root <repo_destino>` -> exit 0 obligatorio. El gate audita AMBAS superficies: rechaza terminal-en-cola-viva, estado-vivo-en-archivo y una etiqueta desconocida colandose como cierre (una errata como `competed` NO pasa por no ser un estado vivo).
   - NO commitees (misma regla que 8 y 8.bis): lo commitea el humano/Manager.
