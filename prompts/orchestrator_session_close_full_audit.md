...
8.bis FUSIoN DEL `backlog_inbox/` (tickets preparados por SESIONES DE DISEnO paralelas). Ademas de tus propios follow-ups (paso 8), fusiona las fichas que una sesion de diseno read-only dejo en `<destination_root>/.agent/collaboration/backlog_inbox/*.tickets.md` (buzon de un-escritor: solo diseno escribe ahi, solo tu -desarrollo- escribes `backlog.md`; asi no hay carrera). Por cada ficha, con el arbol ya limpio y post `--session-close`:
   ...
   - DEDUPE: comprueba que no exista ya una fila equivalente en `backlog.md`; si existe, vincula/absorbe, no dupliques (la ficha declara el ticket vivo relacionado cuando lo hay).
   ...
   - Una ficha con `delivery_authority: repo_motor` (follow-up del motor descubierto en diseno) se registra igual en `backlog.md` del workspace; NUNCA se escribe en `repo_motor`. NO commitees. Contrato del buzon: `research_session_bootstrap.md` (arranque de diseno) y `orchestrator_session_close_full_audit_design.md` (cierre de diseno).
