...
3.5 PORTABILIDAD DEL MOTOR (el motor debe ser AGNOSTICO del destino). Regla escrita que el propio motor ha incumplido en lo que distribuye (WOT-2026-024z): una norma no es un mecanismo. Verifica con probes, no leyendo:
   ...
   - Probe (denominador CERRADO y agujas COMPLETAS -- las dos cosas, o no mide la regla):
     ...
     - **ADVISORY VIVO (no bloquea el cierre; ya fichado):** hay un hardcode conocido en un prompt que viaja -> **WOT-2026-025e**. Reportalo como violacion viva con su ticket; NO lo arregles aqui. Hasta que 025e cierre, **no declares "0 hardcodes"**.
   ...
   - El motor no versiona artefactos de sesion ni output de auditorias (WOT-2026-024y). Probe: `git ls-files destinos/ .agent/runtime/reviews/ .agent/runtime/audit/` -> **0 lineas**. Trackeado == publicado.
     **ADVISORY VIVO (no bloquea; ya fichado en 024y):** hoy este probe sale ROJO por un inventario de sesion trackeado. Reportalo con su ticket; NO lo arregles en el cierre y NO lo declares VERIFICADO.
