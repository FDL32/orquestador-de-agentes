...
# WOT-2026-014a: expected runtime artifacts generated by the session closeout.
# These files are tracked (committed each close) but may be left uncommitted if
# a previous close was interrupted. The closeout pre-push gate should forgive
# them; the GENERAL pre-push gate (default) must NOT forgive them.
# Shared with scripts/closeout_steps/rotation.py (imported there instead of
# being redefined locally) so the two gates always agree on the same set.
EXPECTED_CLOSEOUT_RUNTIME_ARTIFACTS: list[str] = [
...
def check_git_tree_clean(
    ...
    WOT-2026-014a: accepts an optional allowlist of expected runtime artifact
    basenames (e.g. EXPECTED_CLOSEOUT_RUNTIME_ARTIFACTS). When provided, dirty
    lines whose path contains one of those basenames are silently forgiven.
    Default None preserves the original behavior exactly (no forgiveness).

    Args:
        project_root: Raiz del proyecto donde ejecutar git status.
        expected_artifacts: Optional list of basename substrings to forgive.
            Default None == current behavior (any dirty file fails the check).
    ...
    if output_lines:
        # WOT-2026-014a: if an allowlist is provided, filter out expected runtime
        # artifacts before deciding whether the tree is dirty. Lines not matching
        # any expected artifact are still reported as productive dirty changes.
        # With default None the filter is a no-op (current behavior preserved).
        if expected_artifacts:
            output_lines = [
                line
                for line in output_lines
                if not any(pat in line for pat in expected_artifacts)
            ]
        if not output_lines:
            return HygieneResult(
                passed=True,
...
def run_delivery_hygiene_check(  # noqa: C901
    ...
    *,
    ...
    check_git_tree_clean so the closeout pre-push gate can forgive known
    runtime artifacts without weakening the general pre-push gate (default None).
    ...
    Args:
        ...
        expected_artifacts: Optional allowlist forwarded to check_git_tree_clean.
            Default None preserves current behavior (no forgiveness).
    ...
    if check_tree:
        result = check_git_tree_clean(project_root, expected_artifacts)
        results.append(result)
        if not result.passed:
            all_passed = False
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "check_git_tree_clean so the closeout pre-push gate forgives known "
            "runtime artifacts. Does NOT change behavior for any other caller."
        ),
    )

    args = parser.parse_args()

    artifacts = EXPECTED_CLOSEOUT_RUNTIME_ARTIFACTS if args.closeout_mode else None
