...
Objetivo: mantener `orquestacion_agentes/` como motor central único y abrir el proyecto de destino como workspace separado, con su propio `.agent/` mínimo.

### WP-A - Desacople de `project_root` en el motor
- Auditar todas las rutas derivadas de `Path(__file__)` en el motor.
- Parametrizar `project_root` de forma explícita en `agent_controller.py`, `bus/event_bus.py`, `bus/supervisor.py`, `bus/review_bridge.py`, `scripts/manager_review_bridge.py` y `scripts/local_audit.py` como mínimo.
...
### WP-B - Workspace mínimo en el destino
...
- La operación debe seguir funcionando con el mismo contrato canónico de bus y colaboración.

### Regla de secuencia
- Consolidar/publicar primero la base estable actual.
- Después ejecutar WP-A.
- Luego WP-B.
- Finalmente WP-C.
