...
- `scripts/session_closeout.py`: dry-run previews now go to ignored
  ...
  and every child process receives `AGENT_PROJECT_ROOT` explicitly.
- `scripts/session_close_observations.py` and `scripts/memory_consolidate.py`:
  bootstrap `repo_motor` before importing `bus` or `runtime`, allowing
  absolute-path execution with `cwd` in `repo_destino`.
...
- Closeout, observations and memory Model B suite -> 110 passed.
...
### Fixed
- `scripts/manager_review_bridge.py`: bridge startup no longer calls
  `reconcile_state()` in `--watch`/`--once`. It now bootstraps directly from
  persisted `supervisor_state.json`, removing the shared-state write race with a
  freshly launched supervisor during Builder/Manager startup.

### Added
- `tests/test_manager_review_bridge.py`: regression coverage for
  `main --once` using a real persisted supervisor state on disk, verifying the
  bridge reads canonical state without calling `reconcile_state()` at startup.
...
- **Portabilidad**: Cero referencias absolutas a `c:\Users\***REDACTED***` o `z_scripts/` en el repo portable (excepto `observations.jsonl` histórico).
