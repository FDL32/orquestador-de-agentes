...
- `.agent/context/project-map.json`, `.agent/context/` y `.agent/runtime/events/` son artefactos generados que no deben ser tratados como fuente editable por hooks de whitespace o formato.
...
- Los artefactos generados de contexto o sincronizacion (`.agent/context/project-map.json`, `.agent/context/`, `.agent/runtime/events/`) deben quedar excluidos del formateo y del fix automatico.
...
- **Accion:** Crear
- **Descripcion:** Implementar un chequeo determinista que lea `.pre-commit-config.yaml`, ejecute la pasada correctiva de entrega, confirme que `pre-push` no muta archivos y verifique que el arbol queda limpio despues. Debe detectar hooks mutadores en `pre-push`, confirmar que `.agent/context/project-map.json`, `.agent/context/` y `.agent/runtime/events/` estan excluidos y devolver un diagnostico accionable antes del `git push`.
- **Riesgo:** Medio
...
- Los artefactos generados relevantes (`.agent/context/project-map.json`, `.agent/context/`, `.agent/runtime/events/`) quedan excluidos de `pre-commit` y `pre-push`.
