...
1. Las 6 rutas estan trackeadas hoy (verificadas al inicio).
2. Ninguna de las 6 rutas exactas esta en `.gitignore` del motor (solo habia
...
**Referencia de distribucion:** `MANIFEST.distribute` lista las 6 rutas bajo
el comentario `# Collaboration - solo superficies vivas (NO historial)`.
...
Ademas se retiran las 6 lineas de `MANIFEST.distribute` (seccion Collaboration).
...
**Nota: pre-handoff guard no bloquea (WOT-2026-075l no aplica)**

Se verifico en vivo (`motor_checkpoint.assert_work_plan_committed`) que el
pre-handoff guard NO falla con `work_plan.md` destrackeado y gitignorado.
El guard usa `git status --porcelain` sin `--ignored`, asi que un fichero
correctamente gitignorado nunca aparece como "sucio" -- el propio docstring
del guard ya dice explícitamente que `work_plan.md` está exento de deteccion
genérica de dirty-tree. Por tanto no existe ninguna excepcion a la Decision
Arquitectonica (no hay excepcion work_plan.md).

### Rama (B): Mantener trackeadas + mecanismo anti-suciedad -- DESCARTADA

Conservaria historial completo de las 6 rutas pero exigiria diseno de un
...
- **`.gitignore` incluye las 6 rutas exactas.**
- **`MANIFEST.distribute` ya NO declara las 6 rutas.**
...
- **WOT-2026-075l (Alta):** la barrera preventiva contra que `git revert` borre
  ficheros fisicamente sigue siendo un problema real. Solo que no esta
  relacionado con el conflicto de pre-handoff guard (el guard no falla por
  `work_plan.md` gitignorado). Este ticket confirma que no hay excepcion
  necesaria en la decision arquitectonica de destrackear las 6 rutas.
...
- [x] `.gitignore` del motor incluye las 6 rutas exactas.
